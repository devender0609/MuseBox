...
-- v18.8: Cantoa Moments collaboration and scheduled gift drops.
-- Safe to run repeatedly. These tables are service-role only; public access goes through server routes.
create table if not exists public.moment_collections (
...
create index if not exists moment_collections_token_idx on public.moment_collections(token);
alter table public.moment_collections enable row level security;

create table if not exists public.moment_contributions (
  id bigint generated by default as identity primary key,
  collection_id uuid not null references public.moment_collections(id) on delete cascade,
  contributor text not null default 'Someone',
  memory text not null,
  created_at timestamptz not null default now()
);
create index if not exists moment_contributions_collection_idx on public.moment_contributions(collection_id,created_at);
alter table public.moment_contributions enable row level security;

create table if not exists public.song_drops (
  song_id uuid primary key references public.songs(id) on delete cascade,
  owner_id uuid not null references auth.users(id) on delete cascade,
  unlock_at timestamptz not null,
  created_at timestamptz not null default now()
);
create index if not exists song_drops_unlock_idx on public.song_drops(unlock_at);
alter table public.song_drops enable row level security;

insert into public.cantoa_schema_migrations(migration_key)
values('v18.8_cantoa_moments')
on conflict(migration_key) do nothing;

-- v18.8.29: Group Song 2.0 structured contributions, photos and voting.
...
-- v18.8.49: globally persistent throttling for public/unlisted routes and optional Stripe billing-country visibility.
