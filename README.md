...
# v18.8.7 change
Karaoke and Instrumental exports now use independent UI state while sharing one deduplicated backing-track job. Progress clicks no longer intentionally open new tabs, and finished backing audio is reused between both downloads for the current song.

# Cantoa v18.8.7 — Sing & Use state patch

This release adds practical post-generation utility without adding any new primary navigation: finished vocal songs can show follow-along lyrics, build a Karaoke package, or export an instrumental backing track from the existing provider-backed stem separation workflow. Exact lyric timing is not claimed; follow-along highlighting is progress-based until provider timestamps are available. No SQL migration is required.

# Cantoa v18.8.1 — Cantoa Moments

This release adds a progressive-disclosure innovation layer around finished songs: Song DNA, Emotion Lock, Living Songs, Time Machine, Best Moment AI, Memory Capsule, Song Passport, Group Song contribution links, Secret Song Drop, Song Replies, Daily Soundtracks, multilingual duet intelligence, and background cultural/story-to-chorus planning. The primary Create navigation remains unchanged.
...
Cantoa does not present unimplemented DAW, social OAuth publishing, cloned singing voices, referral-credit accounting, or unsupported automatic rendering as active features. My Voice v1 supports consent-based private self-voice cloning for spoken previews/dedications through ElevenLabs; current ElevenLabs Music does not expose a cloned voice as the singer, so Cantoa states that limitation explicitly rather than pretending otherwise.
...
## v18.8.5 — clarity and reuse
- Frozen visible feature set: no new permanent navigation tab or creation mode.
- Cantoa Moments keeps Song DNA/Living Song/Time Machine technical preservation context hidden from the user-facing brief while still sending it to the music pipeline.
- Library adds lightweight search and one compact filter for All, Vocals, Instrumentals and Revised versions.
- Existing Create, Advanced, Revision Studio, Cantoa Moments, Finish & Share, membership, owner console, provider routing and entitlement behavior remain intact.

## v18.8.37
Adds Story Interview, a consolidated multimodal source launcher, voice-memo-to-song-brief flow, direct image/video soundtrack entry points, and optional local gift reaction capture. Existing generation, entitlement, payment, provider-routing, owner-console, and free-tier logic are unchanged.

## v18.8.41
Create-surface simplification and state audit: source helpers are single-select, Website guidance is scoped to Website only, story input is reduced to one free-form moment plus optional vibe chips, starter ideas use progressive disclosure, New Song fully resets source state, informational notices no longer render as errors, and Cantoa video watermarks remain always-on.
