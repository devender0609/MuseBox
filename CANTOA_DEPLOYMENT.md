...
Add `ELEVENLABS_API_KEY` with **Music Generation**, **Speech to Text**, **Voices write/delete**, and **Text to Speech** access if you want My Voice v1 enabled. If the existing restricted key allows only Music, edit or replace it in ElevenLabs before testing **Speak your idea**. Cantoa reserves account minutes before music generation and refunds them if the provider fails.
...
## v18.8.1 Cantoa Moments deployment

1. Deploy the v18.8.1 build normally. Existing creation, library, payment, routing, gift and owner-console flows do not require new environment variables.
2. Run the current rerunnable `supabase-setup.sql` once. The new v18.8 section creates `moment_collections`, `moment_contributions`, and `song_drops`.
3. Verify a signed-in cloud-saved song can create a **Group Song** contribution link; open it in a private browser, submit a memory, then use **Use collected memories** as the creator.
4. Verify **Secret Song Drop** with a future unlock time. Before the scheduled time the share page must not issue a signed audio URL; after the time it opens normally.
5. Verify **Best Moment AI** creates a 15-second Reel from the finished audio without consuming new music-generation minutes.
6. Verify **Memory Capsule**, **Song Passport**, **Song DNA**, **Living Song**, **Time Machine**, and **Song Reply** remain inside the collapsed **More with this song** surface.

No new provider, Stripe, or Vercel environment variables are introduced by v18.8.1.

## v18.8.5 deployment
No Supabase schema change is required. Deploy the app normally. The release changes client-side clarity/reuse behavior only and does not require rerunning `supabase-setup.sql`.

## v18.8.6 Sing & Use deployment

No Supabase migration or new environment variable is required. Deploy normally. The new Lyrics · Sing Along, Karaoke package and Instrumental version actions live only inside the existing Download panel. Karaoke/instrumental reuse the existing `/api/music/stems` integration, so they require the same configured `ELEVENLABS_API_KEY` and eligible provider/stem-separation access already used by Six stems ZIP. Vercel remains the final production typecheck/build verification.


## v18.8.7 Sing & Use state patch
