...
function providerFailureReason(error: unknown) {
  const raw = error instanceof Error ? error.message : "provider attempt failed";
  const flat = raw.replace(/\s+/g, " ").trim();
  const value = flat.toLowerCase();
  if (/quota_exceeded|quota exceeded|credit limit/.test(value)) return "quota_exceeded · primary API-key quota/limit rejected the request";
  if (/too many concurrent|concurrency/.test(value)) return "concurrency_limit · primary provider was at its concurrency limit";
  if (/rate limit|429/.test(value)) return "rate_limited · primary provider asked Cantoa to slow down";
  if (/401|403|unauthorized|forbidden|permission|api key/.test(value)) return "permission_denied · primary provider credentials or endpoint permissions need attention";
  if (/terms of service|policy|copyright|moderation|imitat/.test(value)) return "provider_policy · provider rejected the creative request";
  return flat.slice(0, 150) || "provider attempt failed";
}
  ...
  for (const provider of [...new Set(order)]) {
    try {
      ...
      if (result) return {
        ...
        fallbackReason: result.provider !== first ? previousFailureReason : undefined,
        latencyMs: Date.now() - started,
      };
    } catch (error) {
      lastError = error;
      previousFailureReason = providerFailureReason(error);
      // A creative-policy rejection must not be bypassed by silently trying another provider.
      if (previousFailureReason.startsWith("provider_policy")) {
        const failure = error instanceof Error ? error : new Error("Provider policy rejected this request.");
