...
import { ArrowRight, Check, CircleStop, Headphones, Keyboard, Layers3, Play, Radio, RefreshCw, Sparkles, WandSparkles, Waves } from "lucide-react";
  ...
  return <main className="app-shell" style={{"--c1":world.palette[0],"--c2":world.palette[1],"--c3":world.palette[2],"--c4":world.palette[3]} as React.CSSProperties}>
    ...
    <section className="hero" id="create">
      ...
      <div className="creator-card">
        <div className="card-head"><div><span>NEW PLAYABLE WORLD</span><h2>What do you want to play?</h2></div><b>LIVE PROTOTYPE</b></div>
        <label className="prompt-label" htmlFor="prompt">Describe a place, feeling or impossible idea</label>
        ...
        <div className="control-grid"><label>Energy <strong>{energy}%</strong><input aria-label="Energy" type="range" min="10" max="100" value={energy} onChange={e=>setEnergy(+e.target.value)}/></label><label>Space <strong>{space}%</strong><input aria-label="Space" type="range" min="0" max="100" value={space} onChange={e=>setSpace(+e.target.value)}/></label></div>
        <button className="generate" onClick={generate}><Sparkles/> Create my world <ArrowRight/></button>
        <p className="prototype-note">This prototype generates a procedural instrument instantly—no API key or paid rendering.</p>
      </div>
    </section>
