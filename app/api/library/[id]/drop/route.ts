...
export async function DELETE(request: NextRequest,{params}:{params:Promise<{id:string}>}){
  const user=await authenticatedUser(request);const admin=adminSupabase();if(!user)return NextResponse.json({error:"Sign in required."},{status:401});if(!admin)return NextResponse.json({error:"Cloud sharing is not configured."},{status:503});
  const {id}=await params;const {data:song,error:songError}=await admin.from("songs").select("id").eq("id",id).eq("user_id",user.id).maybeSingle();if(songError)return NextResponse.json({error:"The cloud library could not verify this song right now."},{status:503});if(!song)return NextResponse.json({error:"Song not found."},{status:404});
  const {error}=await admin.from("song_drops").delete().eq("song_id",id).eq("owner_id",user.id);return error?NextResponse.json({error:"Secret Drop could not be cancelled right now."},{status:500}):NextResponse.json({ok:true});
}
