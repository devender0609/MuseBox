...
function publicGenerationError(raw: string) {
  const text = String(raw || "").replace(/\s+/g, " ").trim();
  const value = text.toLowerCase();
  if (/terms of service|policy|copyright|moderation|imitat/.test(value))
  ...
  if (/401|403|unauthorized|forbidden|permission|api key/.test(value))
    return "Cantoa's music connection needs attention. Please try again shortly.";
  if (text.startsWith("{") || text.length > 320) return "The music provider could not complete this request. Try again or adjust the description.";
  return text || "Song generation failed.";
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await logGenerationEvent(request, {
      ...
      status: "success",
      errorCode: result.fallbackUsed ? (result.fallbackReason || "Primary provider attempt failed; compatible fallback completed the request.") : null,
      requestSummary: prompt,
