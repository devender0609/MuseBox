...
test('v18.9.3 package and lockfile agree', () => {
  assert.equal(pkg.version, '18.9.3');
  assert.equal(lock.version, '18.9.3');
  assert.equal(lock.packages[''].version, '18.9.3');
  assert.match(pkg.scripts['test:current'], /cantoa-v18-9-3\.test\.mjs/);
});
...
test('v18.9.3 My Voice ownership is server authoritative', () => {
  const voice = read('app/api/my-voice/route.ts');
  const preview = read('app/api/my-voice/preview/route.ts');
...
test('v18.9.3 sharing controls revoke old URLs and scheduled drops can be cancelled', () => {
...
test('v18.9.3 standardizes audio validation and enables report-only CSP', () => {
...
test('v18.9.3 limits detailed generation diagnostics to a rolling 30 days', () => {
